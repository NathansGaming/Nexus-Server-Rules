...
public final class MobCustomizationListener implements Listener {
    ...
    private void applyProfile(Entity entity, DimensionPreset.MobProfile profile) {
        ...
        applyAttribute(living, Attribute.MAX_HEALTH, profile.healthMultiplier);
        ...
        applyAttribute(living, Attribute.MOVEMENT_SPEED, profile.speedMultiplier);
        applyAttribute(living, Attribute.ATTACK_DAMAGE, profile.damageMultiplier);
        if (profile.scale != 1.0) {
            // Attribute.SCALE - the GENERIC_ prefix was dropped from Attribute in 1.20.5+.
            applyAttribute(living, Attribute.SCALE, profile.scale);
        }
