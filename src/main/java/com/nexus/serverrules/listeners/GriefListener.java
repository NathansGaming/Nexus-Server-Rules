...
import com.nexus.serverrules.integration.LandTrustBridge;
...
public final class GriefListener implements Listener {
    ...
    private final LandTrustBridge landTrust;
    ...
    public GriefListener(JavaPlugin plugin, PunishmentManager punishmentManager, ReviewQueue reviewQueue,
                          int windowSeconds, int blockThreshold, int registryCapacity, LandTrustBridge landTrust) {
        this.plugin = plugin;
    ...
    public void onBreak(BlockBreakEvent event) {
        ...
        if (landTrust.canBuild(player.getUniqueId(), event.getBlock().getLocation())) {
            return; // NexusRealms says this player is currently trusted to build here (teammate,
                     // claim-trusted visitor, etc) - breaking someone else's block on land you're
                     // both legitimately allowed to build on is normal shared building, not grief
        }
