...
This sandbox has no network access to Maven Central / PaperMC's repo, so
I could not run an actual `gradle build` against the real Paper API here
— I verified the source compiles with zero syntax errors by compiling it
without the API present (every error was "cannot find symbol" for Paper
classes, which is expected and disappears once the real dependency is on
the classpath). You'll want to actually build it once before trusting it
fully:

1. Install a JDK 21 and Gradle (or just open the project folder in
   IntelliJ IDEA with the Gradle plugin — it'll handle everything,
   including generating the Gradle wrapper, automatically).
2. `./gradlew build` (or `gradle build` if you don't have a wrapper yet)
   from the project root. The plugin jar lands in `build/libs/`.
