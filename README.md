...
## Patch: a real fuzzy-matcher false positive, found and fixed

A player (`.FireLogan209`) got auto-restricted for `SLUR (FUZZY, root="wetback")`
on the message **"real can you quickly come back to discord"** - which
obviously contains no such term. Root cause, confirmed by reproducing it:

`TextNormalizer` deliberately strips every space before matching (that's
what defeats someone typing `w e t b a c k` to dodge the filter), which
flattened the message to `realcanyouquicklycomebacktodiscord`. The fuzzy
(edit-distance) stage then slid its comparison window across that *entire*
flattened string with no idea where the real word boundaries used to be,
and found `eback` (the tail end of "come" + "back" mashed together) sitting
exactly 2 edits from `wetback` (drop the `w`, drop the `t`) - just inside
the tolerance a 7-letter root gets. Two completely unrelated, innocent
words got smashed together by the normalizer and coincidentally landed in
the slur's fuzzy radius.

**Fix:** fuzzy matching is now restricted to within a single real word
(`TextNormalizer#normalizeWords`, `ViolationDetector#fuzzyMatchesWithinWord`)
- it can no longer slide across a genuine space. EXACT matching is
untouched and still runs on the fully flattened string, so deliberately
spaced-out obfuscation (`wet back`, `w e t b a c k`, `w.e.t.b.a.c.k`) is
still caught immediately, same as before - only the *fuzzy/approximate*
pass, which was never meant to catch spacing tricks in the first place
(that's EXACT's job), had this gap. Verified against five cases: the
false positive above (now clears), a deliberately-split slur (still
EXACT), letter-by-letter spacing (still EXACT), a missing-letter typo
within one real word (still FUZZY), and leetspeak (still EXACT after
substitution) - all five behave correctly now.

If anyone else got auto-restricted for a FUZZY hit before this patch,
it's worth spot-checking `/nexusrules queue` and `violations-log.txt`
for other cross-word coincidences and clearing any that don't hold up -
this exact mechanism could in principle produce other false positives,
not just this one message.

## New in v0.5.2 - NexusRealms integration (claim/team-aware griefing detection)

The problem this patches: griefing-burst detection (`listeners/GriefListener.java`)
only ever knew "someone else placed this block, and you just broke a lot of
those, fast" - it had no idea two players could be legitimately building
together on the same claimed land. On a server running NexusRealms, that
meant teammates - or anyone NexusRealms itself already trusts on a specific
claim - breaking each other's blocks inside their own shared territory got
flagged and auto-restricted as if it were an attack.

**What changed:**
- **NexusRealms** now exposes a small public API
  (`api/NexusRealmsApi.java`, `canBuild(UUID, Location)` /
  `bothCanBuild(UUID, UUID, Location)`), registered with Bukkit's
  ServicesManager on enable. It's backed by a new `land/BuildPermissionService.java`,
  which is the exact same personal-claim-trust → team-membership →
  open-terrain-default logic `ProtectionListener` already enforced for real
  building - pulled out into its own class so the enforcement path and this
  new query API can never quietly drift apart from each other.
- **NexusServerRules** gained `integration/LandTrustBridge.java`, which looks
  up that API **via reflection** (`Class.forName`, then `Method.invoke`) -
  deliberately not a compile-time Maven dependency on NexusRealms, since
  these are two independent plugin projects with no shared build/repo
  between them. This is the same soft-dependency pattern Vault integrations
  use: if NexusRealms isn't installed, the lookup fails once, logs a single
  info line, and `GriefListener` behaves exactly as before (no claim
  awareness, same as v0.5.1). If NexusRealms *is* installed, `GriefListener`
  now skips flagging a break entirely when the breaker currently has build
  trust at that exact location, on top of the existing "natural terrain or
  their own build" exemptions.
- Declared `softdepend: [NexusRealms]` in `plugin.yml` so NexusRealms loads
  first when both are present (the lookup is lazy/cached either way, so load
  order isn't strictly required, just tidier).

**Deliberately NOT included:** the exemption checks the *breaker's* current
trust at that location, not "was the breaker on the same team as the
placer back when it was placed." That's intentional - it's the simpler,
more robust question (matches your actual scenario: same team, same
claimed land, right now), and it naturally also covers a non-team player an
individual claim owner has personally trusted, not just team-territory
membership. The trade-off: if a player is later removed from a team or
claim, they stop being exempt going forward, but a break that already
happened while they were still trusted was already correctly not flagged
at the time - there's nothing to retroactively fix there.

**Still genuinely unverified - same honest caveat as always:** I can't
compile or run either plugin from here (no Maven Central / PaperMC repo
access in this sandbox), so this is read-for-consistency, not
compiler-or-server-verified. You've got a working `mvn clean package`
pipeline now though, so that gap is closable on your end in the way it
wasn't before - build both, and if `canBuild`/`bothCanBuild` or the
reflection lookup don't match up, the compiler (for NexusRealms) or a
`[NexusServerRules] Found NexusRealms but couldn't link to its API` log
line (for NexusServerRules, at runtime) will say so plainly rather than
failing silently.
