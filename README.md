...
A complete, from-scratch Java/Gradle Paper plugin project:
...
**This is a Gradle project — there is no `pom.xml`, and `mvn` will always
fail here with "no POM in this directory."** That's not a bug to patch;
it's just the wrong build tool. Use the bundled Gradle wrapper instead:

1. From the project root: `./gradlew build` (macOS/Linux) or
   `gradlew.bat build` (Windows). The wrapper (`gradlew`, `gradlew.bat`,
   `gradle/wrapper/`) is bundled in this project now, so you do **not**
   need Gradle installed yourself — the first run downloads the exact
   Gradle version this project was built against (8.14.3) and the real
   Paper API from `repo.papermc.io` automatically. That first run needs
   normal outbound internet access; if you're in a locked-down container
   or CI runner with restricted egress, allow `services.gradle.org`,
   `repo.papermc.io`, and `repo.maven.apache.org` (or run it once
   somewhere with open internet and copy the populated `~/.gradle` cache
   over).
2. The plugin jar lands in `build/libs/NexusDimensions-0.1.0.jar`.
3. Copy that jar into your Paper server's `plugins/` folder.

(If you'd rather use a locally installed Gradle instead of the wrapper,
`gradle build` works too — the wrapper just means you don't have to
install one.)

I verified this two ways, from a sandbox with no outbound access to
either of those Maven repos: first, compiling the source with `javac`
directly (no Paper API present) and confirming every resulting error was
an expected "cannot find symbol"/"package does not exist" for a Paper
class, which is exactly what disappears once the real dependency is on
the classpath. Second, actually running `./gradlew build` here — it
correctly parsed the project, resolved the task graph, and got as far as
attempting to download `paper-api` before hitting this sandbox's network
restriction (a 403 from the outbound proxy, not a project problem) — so
the build script itself is confirmed mechanically sound up to the point
this sandbox can't reach the internet. It should complete cleanly the
first time you run it somewhere with normal internet access.
